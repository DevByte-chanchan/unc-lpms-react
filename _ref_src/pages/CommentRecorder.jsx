...
// Standalone approver comment recorder.
// Chains: assignment -> ILO -> coverage type -> target(s) -> message, then POSTs to /api/comments.
// commenter_role is chosen here because composition has no auth/role context yet.
...
export default function CommentRecorder() {
    ...
    async function loadAssignments() {
        try {
            ...
            setToast({ type: 'error', msg: 'Failed to load assignments' });
        }
    }
    ...
    async function onAssignChange(coAssignId) {
        ...
        const pcId = pco.pc_offering_id;
        const revNum = pco.revision_number;
        if (!pcId || !revNum) return;
        try {
            const data = await fetchJson(`/api/ilos/${pcId}/${revNum}`);
            ...
            setToast({ type: 'error', msg: 'Failed to load ILOs for this plan' });
        }
    }
    ...
    async function submit() {
        ...
        try {
            ...
            setToast({ type: 'success', msg: 'Comment saved to the database.' });
            setMessage('');
            setSelectedLabels([]);
        } catch (e) {
    ...
    return (
        <div style={box}>
            ...
            <p style={{ margin: '0 0 18px', fontSize: 13, color: '#64748b' }}>
                Create a review comment against a learning plan and attach one or more targets.
            </p>
