...
// Expected seed-id prefix per course — used to sweep seeds that leaked onto the wrong course
const seedPrefixByCourse = {
  BIT313L: 'seed-hci',
  BIT313: 'seed-hci',
  BSCS322L: 'seed-se',
  BSCS322: 'seed-se',
...
export function seedDummyComments(code) {
  ...
  try {
    ...
    // Sweep: drop seed comments attached to this course that don't belong to it
    // (old fallback used to seed HCI comments onto every course).
    const expectedPrefix = seedPrefixByCourse[code] || null
    const cleaned = list.filter(cm => {
      if (cm.courseCode !== code) return true // other courses untouched
      if (!cm.id || !String(cm.id).startsWith('seed-')) return true // keep manual + server comments
      return expectedPrefix && String(cm.id).startsWith(expectedPrefix + '-')
    })
    if (cleaned.length !== list.length) {
      localStorage.setItem('approval_comments_v1', JSON.stringify(cleaned))
      if (import.meta.env.DEV) console.log('Swept', list.length - cleaned.length, 'stale seed comments for', code)
    }

    // Only seed courses explicitly mapped — every other course gets its real
    // comments from the server via /api/comments/course/:code.
    const gen = courseCommentMap[code]
    if (!gen) return
    if (cleaned.some(cm => cm.courseCode === code && String(cm.id || '').startsWith('seed-'))) return

    const fresh = gen(code)
    localStorage.setItem('approval_comments_v1', JSON.stringify([...fresh, ...cleaned]))
