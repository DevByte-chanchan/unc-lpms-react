 ...
 *
 * Named "Courses" after the table it actually reads — the `course_offerings`
 * table was dissolved into `courses`. "Course Offerings" is still the label
 * users see (nav, title, route), and it remains the correct name for the
 * revisions subsection, which really is Program_Course_Offering data.
 */
...
const ProgramHeadCourses = () => {
  ...
  // while this is set — not by the overlay, and not by the year-level popup it
  // hands off to. The write is CoursesAPI.uploadCommit and nothing else.
  ...
  // Picking a file no longer previews-then-imports on its own — it hands the file
  // to the shared Preview & Confirm overlay, which runs the dry run and shows what
  // is in the file. The year-level popup still happens, but AFTER that step and
  // still BEFORE anything is written. Order is:
  //
  //   pick file → Preview & Confirm → year-level popup (if needed) → uploadCommit → undo toast
  ...
  // Bridges the year-level popup, which is user-driven and lives in JSX, back to
  // the single promise UploadPreviewFlow is awaiting inside commit(). Resolved by
  // onConfirmResolve, rejected if the user backs out of the popup.
  const resolverRef = React.useRef(null);

  // The `commit` UploadPreviewFlow calls when the user clicks "Confirm & Import".
  // It receives the dry run's payload as its 2nd argument, which is what lets the
  // year-level popup open here without paying for a second preview request.
  //
  // Returning a promise is the whole trick: the overlay awaits it, so "commit"
  // can legitimately mean "ask the user some more questions, THEN write".
  const commitCourses = (f, preview) => new Promise((resolve, reject) => {
    const unresolved = (preview && Array.isArray(preview.unassigned)) ? preview.unassigned : [];

    // Every year level recognized — nothing left to ask, so write.
    if (unresolved.length === 0) {
      CoursesAPI.uploadCommit(f, periodId, { programId: selectedProgramId })
        .then(resolve, reject);
      return;
    }

    // Otherwise hand off to the existing reconciliation popup. Nothing is written
    // until it resolves; the overlay stays up behind it, showing "Importing…".
    const init = {};
    unresolved.forEach((u) => { init[u.course_no] = ''; });   // start undecided
    setYearChoices(init);
    setResolveRows(unresolved);
    resolverRef.current = { resolve, reject, file: f };
  });

  // The popup's "Confirm & Import" — turn the user's choices into the resolutions
  // the commit endpoint expects, write, and settle the promise the overlay holds.
  const onConfirmResolve = async () => {
    const pending = resolverRef.current;
    if (!pending || uploading || !resolveRows) return;
    ...
    try {
      const r = await CoursesAPI.uploadCommit(pending.file, periodId, {
        yearLevelOverrides,
        skipCodes,
        programId: selectedProgramId,   // default program for rows with no Program column
      });
      resolverRef.current = null;
      setResolveRows(null);
      setYearChoices({});
      pending.resolve(r);               // → UploadPreviewFlow closes and raises the undo toast
    } catch (err) {
      setUploadError(err.message || 'Upload failed');
      // Leave the popup open so the user can retry — the promise stays pending.
    } finally {
      setUploading(false);
    }
  };

  // Backing out of the year-level popup abandons the import (nothing was written)
  // and drops the user back to the Preview & Confirm step they came from, rather
  // than throwing the whole file away as the old flow did.
  const onCancelResolve = () => {
    ...
    setResolveRows(null);
    setYearChoices({});
    if (pending) pending.reject(new Error('Year levels not set — nothing was imported.'));
  };

  // "Confirm & Import" succeeded (via either branch above).
  const onUploadCommitted = async (result) => {
    refresh();
    setUploadCount((n) => n + 1);
    setPickedFile(null);
    if (uploadInputRef.current) uploadInputRef.current.value = '';
    // Re-open the upload modal on its RESULTS pane — the rich import summary
    // (year breakdown, prerequisites linked, unresolved programs) is this page's
    // own and the toast doesn't replace it.
  ...
  // The dialog is two panes, and they are not the same thing: one asks for a file,
  // the other reports what the import did. It titled both "Upload Course
  // Offerings", which described the pane the user had already finished.
  const uploadTitle = uploadResult ? 'Import changes summary' : 'Upload Course Offerings';

  const closeUpload = () => {
    if (uploading) return;
    // Anything still waiting on the year-level popup is abandoned, not left
    // dangling — an unsettled promise would wedge the overlay on "Importing…".
    const pending = resolverRef.current;
    resolverRef.current = null;
    if (pending) pending.reject(new Error('Import cancelled.'));
    setShowUpload(false); setPickedFile(null); setUploadError(null); setUploadResult(null);
    setResolveRows(null); setYearChoices({}); setPendingFile(null);
  };
  ...
  // Every program that offers this course — one offering per program, each with
  // its own description. A course with none is simply not offered by anyone yet.
  const offerings = (detail && Array.isArray(detail.offerings))
    ? detail.offerings.slice().sort((a, b) => {
        const ac = (a.program && a.program.code) || '';
        const bc = (b.program && b.program.code) || '';
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10, flexShrink: 0 }}>
          ...
          {isCurrentTermActive && (
            <div style={{ display: 'flex', gap: 10 }}>
              <UndoUploadButton entity="courses" periodId={periodId} disabled={!periodId} refreshKey={uploadCount} onUndone={refresh} />
              <button
  ...
  const detailView = headerCourse && (
    <div style={{ flex: '1 1 auto', minHeight: 0, overflowY: 'auto', paddingBottom: 8 }}>
      ...
      <div style={{ marginTop: 28 }}>
        ...
        {offerings.length === 0 ? (
          <div style={{ fontSize: 14, color: SLATE5, fontStyle: 'italic' }}>
            Not offered by any program yet — upload this course under a program to create its offering.
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {offerings.map((o) => (
              <div key={o.pc_offering_id} style={{ border: '1px solid ' + SLATE2, borderRadius: 12, padding: 16, background: '#FFFFFF', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                {/* The offering IS the (course × program) pairing, so the program
                    is the headline. Revision is a detail of that offering. */}
                <div style={{ flexShrink: 0, display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 10px', borderRadius: 8, background: SLATE05, border: '1px solid ' + SLATE2, color: '#18191A', fontSize: 12, fontWeight: 600 }}>
                  <FileText size={13} color={SLATE5} />
                  {(o.program && o.program.code) || 'Unknown program'}
                  <span style={{ color: SLATE3 }}>·</span>
                  <span style={{ color: SLATE5, fontWeight: 500 }}>Rev {o.revision_number}</span>
                </div>
  ...
  const content = (
    <div style={{ padding: 20, background: '#FFFFFF', height: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Both panes of this dialog — pick a file, and the import summary that
          Preview & Confirm hands back to. DialogShell gives it the same fade +
          scale as every other dialog, and it shares the one app backdrop, so the
          preview can fade out into this without the dim blinking between them. */}
      <DialogShell
        open={showUpload}
        onBackdropClick={closeUpload}
        ariaLabel={uploadTitle}
        panelStyle={{ width: 'min(560px, 94vw)', maxHeight: '90vh', overflowY: 'auto', background: '#FFFFFF', borderRadius: 10, padding: 24, display: 'flex', flexDirection: 'column', gap: 16, boxShadow: '0 10px 30px rgba(0,0,0,0.15)' }}
      >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 600, color: '#18191A' }}>{uploadTitle}</div>
            ...
            {!uploadResult ? (
              <>
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  ...
                  <button disabled={!pickedFile} onClick={handleReviewFile} style={{ flex: 1, height: 40, background: '#18191A', border: 'none', borderRadius: 8, color: '#FFFFFF', cursor: pickedFile ? 'pointer' : 'not-allowed', fontWeight: 500, opacity: pickedFile ? 1 : 0.6 }}>Review file</button>
                </div>
              </>
            ) : (
              <>
                <div style={{ fontSize: 14, color: SLATE7, lineHeight: 1.6 }}>
                  ...
                  {/* WHICH courses were added / updated — lists the actual rows so
                      the user can see exactly what changed, not just the counts. */}
                  {['added', 'updated'].map((kind) => {
                    const items = kind === 'added' ? uploadResult.addedCourses : uploadResult.updatedCourses;
                    if (!Array.isArray(items) || items.length === 0) return null;
                    const color = kind === 'added' ? '#047857' : '#1D4ED8';
                    const label = kind === 'added' ? 'Added' : 'Updated';
                    return (
                      <div key={kind} style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid ' + SLATE2 }}>
                        <div style={{ fontSize: 12, fontWeight: 600, color: SLATE7, marginBottom: 6 }}>
                          {label} course{items.length === 1 ? '' : 's'} <span style={{ color }}>({items.length})</span>
                        </div>
                        <div style={{ maxHeight: 168, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 4, paddingRight: 4 }}>
                          {items.map((c, i) => (
                            <div key={c.code + '-' + i} style={{ display: 'flex', gap: 8, alignItems: 'baseline', fontSize: 12.5 }}>
                              <span style={{ fontWeight: 700, color: SLATE9, minWidth: 70, flexShrink: 0 }}>{c.code}</span>
                              <span title={c.title || ''} style={{ color: SLATE7, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.title}</span>
                            </div>
                          ))}
                        </div>
                      </div>
      ...
      {/* "Set year levels" review step. Opens only when the dry run found courses
          whose year level we couldn't recognize, and now runs AFTER the Preview &
          Confirm overlay rather than instead of it. Confirm stays disabled until
          every row has a year or is marked "Don't import" (so nothing is silently
          imported as Unassigned).

          z-index 120/121, not 6/7: it now opens ON TOP of UploadPreviewFlow's
          backdrop (z-index 70). At 6 it would render behind it — present in the
          DOM, invisible on screen, and the import would hang forever waiting on a
          popup the user cannot see.

          Backing out (X / Cancel / backdrop) returns to the preview step via
          onCancelResolve; it no longer throws the whole file away. */}
      {resolveRows && (
        <>
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 120 }} onClick={onCancelResolve} />
          <div role="dialog" aria-modal="true" style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 'min(720px, 96vw)', maxHeight: '90vh', background: '#FFFFFF', borderRadius: 12, zIndex: 121, display: 'flex', flexDirection: 'column', boxShadow: '0 20px 48px rgba(0,0,0,0.22)' }}>
            {/* Header + bulk helper */}
            <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid ' + SLATE2 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                ...
                <button onClick={onCancelResolve} disabled={uploading} aria-label="Close" style={{ background: 'transparent', border: 'none', cursor: uploading ? 'not-allowed' : 'pointer', padding: 4, lineHeight: 0, color: SLATE5, flexShrink: 0 }}><X size={20} /></button>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, padding: '14px 20px', borderTop: '1px solid ' + SLATE2 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                <button onClick={onCancelResolve} disabled={uploading} style={{ height: 40, padding: '0 16px', borderRadius: 6, fontSize: 14, fontWeight: 500, background: '#FFFFFF', color: SLATE7, border: '1px solid ' + SLATE3, cursor: uploading ? 'not-allowed' : 'pointer' }}>Cancel</button>
        ...
        return (
          <EditEntityModal
            ...
            record={{ total_units: hasTotal ? String(totalUnits) : '—', lec_credit: cr.lec, lab_credit: cr.lab, lec_hours: hrs.lec, lab_hours: hrs.lab,
              createdAt: editing.createdAt ?? editing.created_at, updatedAt: editing.updatedAt ?? editing.updated_at }}
            validate={hasTotal ? (v) => {
      ...
      {/* Preview & Confirm → year-level popup → commit → undo toast.
          `commit` here is the handoff: it opens the existing reconciliation popup
          and only writes once the user has resolved every year level. Re-keyed on
          the program, because `commit` closes over selectedProgramId while the dry
          run only re-runs when the FILE changes — switching program mid-review
          would otherwise import against a program the preview never saw. */}
      <UploadPreviewFlow
        key={'courses-preview-' + String(selectedProgramId)}
        file={pendingFile}
        entity="courses"
        periodId={periodId}
        title="Upload Course Offerings"
        preview={(f) => CoursesAPI.uploadPreview(f, periodId, selectedProgramId)}
        commit={commitCourses}
        onBack={() => { setPendingFile(null); setShowUpload(true); }}
        onCancel={() => { setPendingFile(null); setPickedFile(null); if (uploadInputRef.current) uploadInputRef.current.value = ''; }}
...
export default ProgramHeadCourses;
